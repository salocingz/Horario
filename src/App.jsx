...
const COLORS = [
  'bg-blue-50 text-blue-700 border-blue-200','bg-emerald-50 text-emerald-700 border-emerald-200',
  ...
  'bg-indigo-50 text-indigo-700 border-indigo-200','bg-orange-50 text-orange-700 border-orange-200',
  'bg-teal-50 text-teal-700 border-teal-200','bg-pink-50 text-pink-700 border-pink-200',
];
const FIXED_PERIODS = [
...
async function loadFirebase(config) {
  if (!window._fbApp) {
    const [{ initializeApp }, { getFirestore, doc, setDoc, onSnapshot, getDoc }] =
      await Promise.all([
    ...
    window._fbOps      = { doc, setDoc, onSnapshot, getDoc };
  }
  return { db: window._fbFirestore, ...window._fbOps };
}
...
function useFirestore(fbConfig) {
  ...
  React.useEffect(() => {
    ...
    loadFirebase(fbConfig)
      .then(ops => { opsRef.current = ops; setStatus('connected'); })
    ...
    <div className="min-h-screen bg-slate-50 text-slate-700 font-sans">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 px-3 md:px-5 py-2.5 flex items-center justify-between shadow-sm gap-2">
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <div>
            <h1 className="text-sm font-bold text-slate-900 leading-tight">MasterSchedule</h1>
